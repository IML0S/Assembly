...
.data?
    sieveArray BYTE 65001 DUP(?)      ; index = number 의미로 65000개 + 1

.code
main PROC
    INVOKE InitArray, ADDR sieveArray, 65001
    INVOKE Sieve, ADDR sieveArray, 65001
    INVOKE PrintPrimes, ADDR sieveArray, 65001
    exit
main ENDP
;---------------------------------------------------------------
...
InitArray ENDP
;---------------------------------------------------------------
Sieve PROC USES esi edi ebx ecx edx eax,
    pArray:PTR BYTE,
    count:DWORD
; 에라토스테네스의 체
; sieveArray[i] = 1이면 합성수
;---------------------------------------------------------------
    mov esi, pArray
    ...
    mov ebx, 2                  ; 테스트할 수 ebx = 2

OuterLoop:
    mov eax, ebx
    imul eax, eax               ; eax = ebx^2
    cmp eax, count
    jae EndSieve                ; ebx^2 >= limit → 종료
    ...
    ; -------------------------
    mov ecx, ebx
    imul ecx, ecx               ; ecx = ebx*ebx (첫 배수)

MarkLoop:
    cmp ecx, count
    jae SkipPrime

    mov BYTE PTR [esi + ecx], 1     ; 합성수 마킹
