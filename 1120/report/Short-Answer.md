...
**Answer:** LEA can perform complex address calculations at runtime.  
Example:
```
lea eax, [ebx + ecx*2]
```
...
**Answer:** False.  
Reason: 매개변수는 여러 줄에 나누어 작성해도 된다.
...
**Answer:** False.  
Reason: MASM은 타입 검사를 강하게 하지 않아 컴파일 타임 오류가 발생하지 않는다.
...
**Answer:** True.  
Reason: 참조 매개변수는 유효한 메모리 주소가 필요하며 즉시값은 잘못된 주소가 되어 런타임 오류가 발생할 수 있다.
